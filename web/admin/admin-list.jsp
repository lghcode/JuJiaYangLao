...
<div class="page-container">
	...
	<table class="table table-border table-bordered table-bg">
		...
		<c:forEach items="${requestScope.accounts}" var="account" varStatus="num">
			<tr class="text-c">
				<td><input type="checkbox" value="${account.aid}" name="ids"></td>
				<td>${account.aid}</td>
				<td>${account.alogin}</td>
				<td>${account.aphone}</td>
				<td>${account.aemail}</td>
				<c:if test="${account.arank==0}">
					<td>超级管理员</td>
				</c:if>
				<c:if test="${account.arank==1}">
					<td>编辑</td>
				</c:if>
				<td>${account.areal}</td>
				<td class="td-manage"><a title="编辑" href="javascript:;" onclick="admin_edit('管理员编辑','Account-get.action?account.aid=${account.aid}','1','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_del(this,${account.aid})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
